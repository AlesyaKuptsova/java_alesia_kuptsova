...
public class TourManager {
    ...
    public void loadTours(TourStorage storage) {
        try {
            this.tours = storage.load();
        }
        catch(TourStorageException exc) {
            throw new TourManagerException(String.format("tours load failed: %s", exc.getMessage()));
        }
    }

    public void storeTours(TourStorage storage) {
        try {
            storage.store(tours);
        }
        catch(TourStorageException exc) {
            throw new TourManagerException(String.format("tours store failed: %s", exc.getMessage()));
        }
    }
